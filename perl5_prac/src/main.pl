...
# Applies to perl 5. NOTE: there is a perl 6 (raku) out aswell
...
sub array_examples {
  ...
  push @ages, 40;
  # concatenate lists
  @more_duped_ages = (25, 30, 40);
  push @ages, @more_duped_ages;
  # .map(), .filter() (basic), .reduce()
  $summed_aged_plus1_to_each = reduce {$a + $b} (map {$_ + 1} (uniq @ages));
  print "summed_aged_plus1_to_each: $summed_aged_plus1_to_each";
}

sub while_loop_examples {
  my $counter = 10;
  while($counter > 0){
    print("while loop counter value: $counter\n");
    $counter--;
  }
}
...
# objects - class, abstract class
sub objects {
  $person = new Person("Bob", 23234345);
  $person->details;
  $person->breathe;
}
